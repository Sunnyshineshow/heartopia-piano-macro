...
def parse_midi(midi_path: str, bpm_scale: float = 1.0):
    """Parse MIDI and return list of (abs_time_seconds, [keys]) chord groups."""
    mid = mido.MidiFile(midi_path)
    tempo = 500000
    ticks_per_beat = mid.ticks_per_beat

    # Collect all (time, key) pairs first
    raw = []
    abs_time = 0.0
    for msg in mido.merge_tracks(mid.tracks):
        ...
        if msg.type == 'note_on' and msg.velocity > 0:
            ...
            if key:
                raw.append((abs_time / bpm_scale, key))

    # Group notes that occur within 20ms of each other into chords
    CHORD_WINDOW = 0.020
    chords = []
    i = 0
    while i < len(raw):
        ...
        group = [k]
        j = i + 1
        while j < len(raw) and abs(raw[j][0] - t) <= CHORD_WINDOW:
            group.append(raw[j][1])
            j += 1
        chords.append((t, group))
        i = j
...
def _press_key(key: str):
    """Press and release a single key (run in a thread for chords)."""
...
def play(chords: list, countdown: int = 5):
    """Play chord events using pydirectinput with precise timing and threading."""
    try:
        import pydirectinput
    except ImportError:
    ...
    for abs_time, keys in chords:
        # Busy-wait for precise timing
        while time.perf_counter() - start < abs_time:
            pass
        if len(keys) == 1:
            # Single note — no threading overhead needed
            _press_key(keys[0])
        else:
            # Chord — fire all keys simultaneously in threads
            threads = [threading.Thread(target=_press_key, args=(k,)) for k in keys]
            for t in threads:
                t.start()
            for t in threads:
                t.join()
...
def save_playback_script(chords: list, out_path: str, bpm_scale: float):
    """Save a standalone Python playback script with chord + threading support."""
    total_notes = sum(len(keys) for _, keys in chords)
    lines = [
        '"""',
    ...
    lines += [
        ...
        '        press(keys[0])',
        '    else:',
        '        threads = [threading.Thread(target=press, args=(k,)) for k in keys]',
    ...
    print(f"Chords  : {sum(1 for _, keys in chords if len(keys) > 1)}")
    print(f"Duration: {chords[-1][0]:.1f}s")
